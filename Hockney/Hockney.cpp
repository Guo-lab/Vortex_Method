...
{ 
  shared_ptr<FFTW1D> p_fftw1d = shared_ptr<FFTW1D>(new FFTW1D(a_M+1));
...
void Hockney::convolve(RectMDArray<double>& a_rhs)
{
  ...
  // cout << "central value of kernel" << real(kernel[getZeros()]) << endl;
  // copyReal(kernel,realOut);
  //MDWrite("kerArray",realOut); 
  m_fftmd.forwardCCcen(rhsDouble);
  // copyReal(rhsDouble,realOut);
  // MDWrite("realRhsHat",realOut);
  // copyImag(rhsDouble,realOut);
  // MDWrite("imagRhsHat",realOut);
  m_fftmd.forwardCCcen(kernel);
  // copyReal(kernel,realOut);
  // MDWrite("realKerHat",realOut);
  // copyImag(kernel,realOut);
  // MDWrite("imagKerHat",realOut);
