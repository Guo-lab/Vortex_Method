...
void PWrite(const char *a_filename, const ParticleSet *a_p) {
    ...
    unsigned int size = a_p->getSize();
    std::vector<float> x(3 * size);
    for (unsigned int i = 0; i < size; i++) {
        const Particle &p = a_p->getParticle(i);
        x[i * 3] = p.m_x[0];
        x[i * 3 + 1] = p.m_x[1];
#if DIM == 3
        x[i * 3 + 2] = p.m_x[2];
    ...
    write_point_mesh(a_filename, 0, size, &(x[0]), 0, 0, 0, 0);
}
...
void MDWrite(const char *filename, RectMDArray<double> &a_array) {
    DBox bx = a_array.getDBox();
    RectMDArray<float> array(bx);
    for (int k = 0; k < bx.sizeOf(); k++) {
        Point pt = bx.getPoint(k);
        array[pt] = (float)a_array[pt];
    }
